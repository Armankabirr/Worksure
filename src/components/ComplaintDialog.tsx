...
export const ComplaintDialog = ({ 
  ...
  const [errors, setErrors] = useState<{category?: string; subCategory?: string; description?: string}>({});

  // Validation
  const validateForm = () => {
    const newErrors: {category?: string; subCategory?: string; description?: string} = {};
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (otherPartyId) {
        formData.append("againstUserId", otherPartyId);
      }
      ...
      await axiosPublic.post("/userRoutes/createComplaint", formData, {
        headers: {
